...
foreach (char_devices) {
	driver"devc-serusb -d vid=$(vendor_id),did=$(product_id),busno=$(busno),devno=$(devno),ign_remove";
}
...
--        inc_usr_spec_id = unique"/fs/ipod";  -- does rsrcdbmgr_devno_attach
        driver"io-audio -dusb busno=$(busno),devno=$(devno),cap_name=ipod-$(busno)-$(devno)";
    };
    class(USB_CLASS_HID) {
        custom = function(obj)
            don = device_obj_name(obj);
            pps_publish(don, "iap::2");
...
device() {
    class(USB_CLASS_COMMS, USB_COMMS_SUBCLASS_NCM) {
        custom_start = function(obj)
            name = "ncm";
            iface = name .. "0";
            mount_cmd = "mount -Tio-pkt -o verbose=0,name=" .. name .. ",path=/dev/otg/io-usb,protocol=ncm,usbdnet_mac=000022446688,mac=020022446688 /ut_carplay/armle-v7/lib/dll/devnp-usbdnet.so";
            ...
            if iface_ready == true then
                ...
                pps_publish(obj.netiface_drv, "NCM::start");
                return"$(start_cmd)";
            else
        ...
        custom_removal = function(obj)
            pps_publish(obj.netiface_drv, "NCM::stop");
