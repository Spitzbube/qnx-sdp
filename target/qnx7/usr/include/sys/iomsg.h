...
struct _io_connect {
	...
	_Uint16t				dirfd;          /* Directory for *at() functions */
	_Uint16t				path_len;
...
struct _io_space {
	_Uint16t					type;
	_Uint16t					combine_len;
	_Uint16t					subtype;			/* F_ALLOCSP, F_GROWSP, F_FREESP */
	short						whence;
	_Uint64t					start;
...
/*
 * resmgr must set this bit to indicate that it is using the extended
 * functionalities.
 */
#define _IO_MMAP_REPLY_FLAGS_USE_EXTENDED           (1U << 0)

/*
 * resmgr control over cache invalidating (default behaviour is to let the
 * memmgr decide, which matches the old behaviour) (bits 1 and 2)
 */
#define _IO_MMAP_REPLY_FLAGS_CACHE_DEFAULT            (0U)
#define _IO_MMAP_REPLY_FLAGS_CACHE_FORCE_INVALIDATE   (1U << 1)
#define _IO_MMAP_REPLY_FLAGS_CACHE_READ_FROM_CACHE    (1U << 2)
#define _IO_MMAP_REPLY_FLAGS_CACHE_DEFER_TO_MEMMGR    ((1U << 1) | (1U << 2))
#define _IO_MMAP_REPLY_FLAGS_CACHE_MASK               ((1U << 1) | (1U << 2))

/* bypass fstat() call (bits 3 and 4) */
#define _IO_MMAP_REPLY_FLAGS_STAT_FORM_MASK           (_STAT_FORM_MASK << 3)
#define _IO_MMAP_REPLY_FLAGS_STAT_FORM_TO_FLAGS(form) (((form) << 3) & _IO_MMAP_REPLY_FLAGS_STAT_FORM_MASK)
	// yields _STAT_FORM_T32_2001 (1)
	//        _STAT_FORM_T32_2008 (2)
	//        _STAT_FORM_T64_2008 (3)
#define _IO_MMAP_REPLY_FLAGS_STAT_FORM(flags)        (((flags) & _IO_MMAP_REPLY_FLAGS_STAT_FORM_MASK) >> 3)

/* bypass fstatvfs() call (bits 5 and 6) */
#define _IO_MMAP_REPLY_FLAGS_BYPASS_FSTATVFS          (1U << 5)
#define _IO_MMAP_REPLY_FLAGS_REMOVABLE                (1U << 6)

/* server wants client to map one of its shmem objects (value is in fd field) */
#define _IO_MMAP_REPLY_FLAGS_SERVER_SHMEM_OBJECT      (1U << 7)
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/c/public/sys/iomsg.h $ $Rev: 897926 $")
#endif
