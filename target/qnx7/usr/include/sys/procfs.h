...
typedef debug_aspace_t			procfs_asinfo;
...
/* these map to their ABLE_* counterparts except the UNCREATED + SUBRANGE value */
#define PROCFS_ABLE_ALLOW_ROOT			0x0001u
#define PROCFS_ABLE_ALLOW_NONROOT		0x0002u
#define PROCFS_ABLE_DEFAULT_ROOT		0x0200u
#define PROCFS_ABLE_DEFAULT_NONROOT		0x0400u
#define PROCFS_ABLE_LOCK				0x0040u
#define PROCFS_ABLE_INHERIT				0x0080u
#define PROCFS_ABLE_MASK				(PROCFS_ABLE_ALLOW_ROOT | PROCFS_ABLE_ALLOW_NONROOT | \
										PROCFS_ABLE_DEFAULT_ROOT | PROCFS_ABLE_DEFAULT_NONROOT | \
										PROCFS_ABLE_LOCK | PROCFS_ABLE_INHERIT)
#define PROCFS_ABLE_SUBRANGE			0x4000u /* ability has subranges defined */
#define PROCFS_ABLE_UNCREATED			0x8000u /* dynamic ability hasn't been created yet */

typedef struct __attribute__((packed)) _procfs_ability_range {
	_Uint64t	lo;		/* range lower boundary */
	_Uint64t	hi;		/* range higher boundary */
	_Uint16t	id;		/* ability index number */
	_Uint16t	able;	/* ability flags */
}							procfs_ability_range;

typedef struct __attribute__((packed)) _procfs_abilities {
	_Uint32t					nbytes;		/* number of bytes to pack all data */
	_Uint16t					snables;	/* number of static abilities */
	_Uint16t					dnables;	/* number of dynamic abilities */
	_Uint32t					nranges;	/* number of ranges */
	_Uint16t					eol_flags;	/* eol ability flags */
	__FLEXARY(_Uint16t, ables);				/* abilities */
	/* procfs_ability_range		ranges[];	ability ranges for this process */
}							procfs_abilities;

/* calculates the amount of space required to store all a process' abilities and subranges */
#define PROCFS_ABLE_SIZE(n) (sizeof(procfs_abilities) + ((n)-1)*sizeof(_Uint16t))
#define PROCFS_ABLE_TOTAL_SIZE(n, r) (PROCFS_ABLE_SIZE(n) + (r)*sizeof(procfs_ability_range))
...
/*
 * Retrieve information about the address space of a process.
 */
#define DCMD_PROC_ASINFO		(__DIOF(_DCMD_PROC, __PROC_SUBCMD_PROCFS + 30, procfs_asinfo))

/*
 * Free:  (__PROC_SUBCMD_PROCFS + 31) - (__PROC_SUBCMD_PROCFS + 33)
 */
...
/*
 * This call will return all the process' abilities and ranges
 */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/system/public/sys/procfs.h $ $Rev: 879551 $")
#endif
