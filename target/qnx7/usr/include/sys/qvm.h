...
enum qvm_msg_subtypes {
	...
	QVM_VERSION,
	/** See ::qvm_guest_ctx */
	QVM_GUEST_CTX_READ,				/**< Read OS maintained guest context (FPU) */
	QVM_GUEST_CTX_WRITE,			/**< Write OS maintained guest context (FPU) */
};
...
enum qvm_control_operations {
	...
	QCO_ARM_GET_VHE_ENTRY  = QCO_ARCH_SPECIFIC, /**< Get VHE guest entry point */
};
...
/**
 * Layout of the ::QVM_GUEST_CTX_READ/WRITE message.
 */
struct qvm_guest_ctx {
	/** Must be ::_QVM_BASE */
	_Uint16t				type;
	/** Must be ::QVM_GUEST_CTX_READ/WRITE */
	_Uint16t				subtype;
	/** Context type, must be zero (FPU) for right now */
	_Uint16t				ctx;
	/** Thread id of virtual cpu thread to use for context */
	_Uint16t				vctid;
	/** Starting offset of the ctx to read/write */
	_Uint32t				offset;
	/** Number of bytes of the ctx to read/write */
	_Uint32t				nbytes;
	/* Followed by context if ::QVM_GUEST_CTX_WRITE */
};
...
union qvm_op {
	...
	struct qvm_guest_ctx		 		guest_ctx;
};


#define QVM_GUEST_ID_NONE	(~(uint64_t)0)

#define QVM_LOAD_GUEST_FPU	((_Uintptrt)1)
#endif


#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/system/public/sys/qvm.h $ $Rev: 912678 $")
#endif
