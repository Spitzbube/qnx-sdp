...
enum {
	_MEM_SHMEM_CREATE_HANDLE,
	_MEM_SHMEM_OPEN_HANDLE,
	_MEM_SHMEM_DELETE_HANDLE,
};

enum {
	_MEM_SHMEM_REVOKE_ALL,
	_MEM_SHMEM_REVOKE_ONE,
};
...
typedef union {
	struct _mem_shmem_handle	i;
	struct _mem_shmem_handle	o;
} mem_shmem_handle_t;

struct _mem_shmem_revoke {
	_Uint16t	type;		// _MEM_SHMEM_REVOKE
	_Uint16t	subtype;	// _MEM_SHMEM_REVOKE_ONE or _MEM_SHMEM_REVOKE_ALL
	_Int32t		pid;		// PID that mapped the object that needs access revoked
	_Uint64t	zero64[4];	// future
};

typedef union {
	struct _mem_shmem_revoke i;
} mem_shmem_revoke_t;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/system/public/sys/memmsg.h $ $Rev: 905299 $")
#endif
