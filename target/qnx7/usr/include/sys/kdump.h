/*
 *  sys/kdump.h
 *
 */
#ifndef __KDUMP_H_INCLUDED
#define __KDUMP_H_INCLUDED

#ifndef __PLATFORM_H_INCLUDED
#include <sys/platform.h>
#endif


#define WRITE_CMD_ADDR	((void *)-(uintptr_t)1)
#define WRITE_INIT		0	/* Dump is about to begin */
#define WRITE_FINI		1	/* Dump is complete */
#define WRITE_PREINIT	2	/* System boot, for e.g. memory allocation */

#define KDUMP_SYSTEM			0x00000000
#define KDUMP_ACTIVE			0x00000001
#define KDUMP_ALL				0x00000002

#define KDUMP_IFS_BOOTSTRAP		0x00000000
#define KDUMP_IFS_FULL			0x00000010

#define KDUMP_MEM_MASK			0x0000000F
#define KDUMP_IFS_MASK			0x000000F0

struct kdump {
	int             dump_type;
	int				kdebug_wanted;
	int				compress;
	int				big;
	int				(*writer)(void const *, unsigned);
	unsigned		spare[3];
	unsigned		kp_size;
	unsigned		kp_idx;
	char			kp_buff[1]; /* variable sized, determined by 'size' field */
};

#define NOTE_VERSION_CURRENT	0

struct kdump_note {
	_Uint8t			note_version;
	_Uint8t			sig_num;
	_Uint8t			sig_code;
	_Uint8t			fault_num;
	_Uint16t		num_cpus;
	_Uint16t		faulting_cpu;
	_Uint32t		regset_size; /* CPU registers + extra registers */
	_Uint32t		cpu_info;
	_Uint32t		procnto_segnum;
	_Uint32t		syspage_segnum;
	_Uint32t		dumpinfo_segnum;
	_Uint32t		cpu_info_hi;
	_Uint32t		extra_reg_value_size;
	_Uint32t		extra_reg_name_size;
	/* Followed by set of strings of extra reg names */
	/* Followed by array of 'num_cpus' struct kdump_note_cpu's */
};

struct kdump_note_cpu {
	_Paddr64t		pgtbl;
	_Uint32t		cpunum;
	_Uint32t		spare[4];
	char			process_name[100];
	/* Followed by CPU register set */
	/* Followed by extra register set */
};

#endif

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/kdumper/public/sys/kdump.h $ $Rev: 898231 $")
#endif
