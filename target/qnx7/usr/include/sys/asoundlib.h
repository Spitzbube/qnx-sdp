...
int snd_ctl_ducking_status_read(snd_ctl_t *handle, const char* ducking_output, snd_ducking_priority_status_t **info );
...
int snd_pcm_set_filter(snd_pcm_t *handle, int channel, const snd_pcm_filter_t * filter);
...
int snd_pcm_channel_drop (snd_pcm_t *pcm, int channel);
...
int snd_pcm_link_mode (snd_pcm_t *pcm, int mode);
int snd_pcm_link_transition (snd_pcm_t *pcm);
...
const char *snd_pcm_get_chmap_channel_name(enum snd_pcm_chmap_position chmap_position);
#ifdef __cplusplus
}
#endif
...
int snd_pcm_plugin_drop (snd_pcm_t *pcm, int channel);
int snd_pcm_plugin_drain (snd_pcm_t *pcm, int channel);
...
int snd_afm_set_path(snd_afm_t *handle, int type, const char *path);
int snd_afm_get_audio_mode(snd_afm_t *handle, char *mode, int size);
int snd_afm_set_audio_mode(snd_afm_t *handle, const char *mode);
int snd_afm_get_vin_list_count(snd_afm_t *handle, int *num);
int snd_afm_get_vin_list(snd_afm_t *handle, snd_afm_vin_list_item_t *list, int num);
int snd_afm_set_vin_stream(snd_afm_t *handle, snd_afm_vin_pair_t *stream, int num);
...
int snd_afm_set_param(snd_afm_t *handle, uint32_t id, uint32_t size, const void* data);
...
int snd_pcm_load_apx_dataset(snd_pcm_t *pcm, uint32_t apx_id, const char *dataset, int *ap_status);
int snd_pcm_set_apx_external_volume(snd_pcm_t *pcm, int nVoices, int16_t *vol_mB);
int snd_pcm_set_apx_user_volume(snd_pcm_t *pcm, int16_t vol_mB);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/asound/public/include/sys/asoundlib.h $ $Rev: 910519 $")
#endif
