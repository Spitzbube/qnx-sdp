 ...
 *          EACCES		caller does not meet security options set (see SCHED_APS_ADD_SECURITY).
 ...
 * 			range (2 to 400).
 *		EACCESS: if SCHED_APS_SEC_PARTITIONS_LOCKED is set; or if SCHED_APS_SEC_ROOT0_OVERALL is set, and you aren't running
 *			in the system partition with the PROCMGR_AID_APS_ROOT ability.
 *
 */
typedef struct {
 ...
 * setting SCHED_APS_SEC_RECOMMENDED. Unless you are testing partitioning and want to change all parms without
 * needing to restart, at least BASIC security should be set. You can use the SCHED_APS_ADD_SECURITY command
 * to SchedCtl(), or the aps command line utility. For example:
 *
 *      sched_aps_security_parms p;
 *      APS_INIT_DATA(&p);
 *      p.sec_flags = SCHED_APS_SEC_RECOMMENDED;
 *      ret = SchedCtl(SCHED_APS_ADD_SECURITY, &p, sizeof(p));
 *
 *
 ...
 * -------------------------
 * Changing overall scheduling parameters, creating and modifying partitions require PROCMGR_AID_APS_ROOT and
 * the caller must be running in the system partition. This arranges for a 2 level hierarchy of partitions:
 * the system partition and its children. Only a thread running in the system partition may join another
 * partition. Percentage budgets must not be zero.
 *
 * SCHED_APS_SEC_FLEXIBLE
 * ----------------------
 * Changing overall scheduling parameters requires PROCMGR_AID_APS_ROOT and the caller must be running in the
 * system partition. Creating partitions requires PROCMGR_AID_APS_ROOT. Changing partition parameters requires
 * PROCMGR_AID_APS_ROOT and the caller must be running in the parent partition of the partition being modified.
 * To join threads into a partition the caller must be running in the parent partition of the partition being
 * joined. This permits applications to create their own local sub-partitions out of their own budgets.
 * Percentage budgets must not be zero.
 *
 * SCHED_APS_SEC_BASIC
 * -------------------
 * Changing overall scheduling parameters requires PROCMGR_AID_APS_ROOT and the caller must be running in the
 * system partition. Setting critical budgets requires PROCMGR_AID_APS_ROOT.
 *
 * In general, RECOMMENDED > FLEXIBLE > BASIC. Note all three allow partitions to be created and modified. To
 * prevent further unauthorized changes after seting up partitions use SCHED_APS_SEC_PARTITIONS_LOCKED. Example:
 *
 *      sched_aps_security_parms p;
 *      APS_INIT_DATA(&p);
...
/* You must have PROCMGR_AID_APS_ROOT and be in the system partition to change overall scheduling parameters,
 * such as the averaging windowize.
 */

#define SCHED_APS_SEC_ROOT_MAKES_PARTITIONS		0x00000002
/* You must have PROCMGR_AID_APS_ROOT to create or modify partitions. Applies to:	SCHED_APS_CREATE_PARTITION,
 * SCHED_APS_MODIFY_PARTITION.
 */
...
/* PROCMGR_AID_APS_ROOT is required to create a non-zero critical budget, or change and existing critical budget*/
...
/* You must have PROCMGR_AID_APS_ROOT to join a thread to a partition */
 ...
 *
 * You must have PROCMGR_AID_APS_ROOT and be in the system partition to call this, even if all security options are off.
 *
 * returns:		EOK, ENOSYS, EINVAL, EACCES, EDOM as described for SchedCtl
 *
 */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/system/public/sys/sched_aps.h $ $Rev: 874585 $")
#endif
