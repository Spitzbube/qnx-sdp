/*
 * $QNXLicenseC:
 * Copyright 2005,2018 QNX Software Systems. All Rights Reserved.
 *
...
// USBD_VERSION - versions the interface between the client and libusbd[c]i
...
#define USBD_BUS_ENABLE				1
#define USBD_BUS_DISABLE			0
...
#define USBD_HCD_CAP_BULK_STREAM		0x00020000
#define USBD_HCD_CAP_SUPER_SPEED_PLUS		0x00040000
...
typedef enum _usbd_override_desc_flag {
	USBD_OVERRIDE_DESCRIPTOR_INTERRUPT_IN_POLL_INTERVAL = 1,

	USBD_OVERRIDE_DESCRIPTOR_LAST
} usbd_override_desc_flag_e;
...
typedef struct usbd_hcd_info {
	...
	_Uint8t					controller_type;
	_Uint8t					controller_state;
	_Uint8t					max_streams;		/* Max streams is loosely based on how this field is
												   reported by hardware controllers.  A macro is
												   supplied for conversion.*/
	_Uint8t					_reserved[9];
} usbd_hcd_info_t;

#define USBDI_HCD_INFO_MAX_STREAMS_TO_NSTREAMS( max_streams ) ( max_streams?( 1 << ( max_streams + 1 ) ) : 0 )
...
typedef struct _usbd_override_desc {
	usbd_device_instance_t		instance;	// device instance for whose descriptor to override
	usbd_override_desc_flag_e	flag;
	usbd_descriptors_t		descriptor;
} usbd_override_desc_t;
...
extern int					usbd_set_bus_state(struct usbd_connection *connection, _uint8 bus, _uint8 state);
...
extern int					usbd_abort_stream(struct usbd_pipe *pipe, struct usbd_urb *purb);
...
extern int					usbd_smmu_map( uint64_t addr, uint64_t len );
extern int					usbd_smmu_unmap( uint64_t addr, uint64_t len );
...
extern int					usbd_override_device_desc_rule( struct usbd_connection *connection, usbd_override_desc_t *override );
...
extern int					usbd_setup_bulk_stream(struct usbd_urb *urb, _uint32 flags, void *addr, _uint32 len, _uint32 stream_id);
...
extern int					usbd_setup_sg_bulk_stream(struct usbd_urb *urb, _uint32 flags, usbd_urb_sg_elem_t *sg_elems, int sg_count, _uint32 stream_id );
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/usbdi/public/sys/usbdi.h $ $Rev: 912097 $")
#endif
