...
/*
 * The _NAME_FLAG_COF flags are in sync with the _CONNECT_CTRL_COF ones from
 * fcntl.h, except CANCELLATION_POINT, but are shifted left by four bits (bits
 * 0/1/3 are already taken by other NAME_FLAG_ symbols).
 */

#define _NAME_FLAG_COF_SHIFT		(4)
#define _NAME_FLAG_COF_CLOEXEC		(_CONNECT_CTRL_COF_CLOEXEC     << _NAME_FLAG_COF_SHIFT)
#define _NAME_FLAG_COF_INSECURE		(_CONNECT_CTRL_COF_INSECURE    << _NAME_FLAG_COF_SHIFT)
#define _NAME_FLAG_COF_NOEVENT		(_CONNECT_CTRL_COF_NOEVENT     << _NAME_FLAG_COF_SHIFT)
#define _NAME_FLAG_COF_REG_EVENTS	(_CONNECT_CTRL_COF_REG_EVENTS  << _NAME_FLAG_COF_SHIFT)
#define _NAME_FLAG_CALLER_FLAG_MASK	((_CONNECT_CTRL_CALLER_FLAG_MASK >> 1) << (_NAME_FLAG_COF_SHIFT+1))
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/BC700_6762_sdp704/lib/c/public/sys/dispatch.h $ $Rev: 882738 $")
#endif
