...
#define  DISCARD_OUTPUT		 0x0000010	 /* Used to discard all output written to the device */
...
#define dev_lock(dev)		(tty_lock(&ttyctrl))
#define dev_unlock(dev)		(tty_unlock(&ttyctrl))
...
extern int  dev_enable_perm(TTYCTRL *ctrl);
extern int  dev_init_lock(TTYCTRL *ctrl);
extern int  tty_lock(TTYCTRL *ctrl);
extern int  tty_unlock(TTYCTRL *ctrl);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/lib/io-char/public/sys/io-char.h $ $Rev: 895107 $")
#endif
