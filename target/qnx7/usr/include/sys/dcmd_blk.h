...
#define DCMD_FSYS_ADDITY_CTL		__DIOTF(_DCMD_FSYS, 37, struct addity_ctl)
#define DCMD_FSYS_DIRECT_IO_IOV		__DIOT(_DCMD_FSYS,  38, struct fs_directio_iov)

// new direct I/O (V) devctl code for additional piggyback data (ICE_TRANS_INFO) at the end of the structure.
#define DCMD_FSYS_DIRECT_IO_IOV_ICE		__DIOT(_DCMD_FSYS,  40, struct fs_directio_iov)

/* version flag to query, version value is returned */
enum {
	FSYS_VERSION_ROLLBACK = 0x1,
};
...
#define FS_DIO_MAP_PHYS			(0x80000000u)	// perform virtual to physical mapping internally.
#define FS_DIRECTIO_MAX_BMAP	128
#define FS_DIRECTIO_MAX_IOV		256
...
struct fs_directio_iov {
	off64_t			offset;					// file offset
	_Uint32t		nbytes;					// total length
	_Uint32t		flags;					// FS_DIO.*
	_Uint32t		niov;					// count of io parts
	__FLEXARY(struct __iovec64, iov);		// (struct __iovec64 iov[]) io parts
};
...
/*	Argument structure for devctl(DCMD_FSYS_ADDITY_CTL).
 */
typedef struct addity_ctl {
	_Uint32t  op;              /* one of DCMD_FSYS_ADDITY_CTL_OP_.. */
	union {
		_Uint8t  data[1020];   /* sizeof(addity_ctl_t) == 1024 */
		struct {
			char  name[256];   /* name of partition/disk to add to addity */
		} add;
	};
} addity_ctl_t;

/*	Command codes for devctl(DCMD_FSYS_ADDITY_CTL).
 */
enum {
	DCMD_FSYS_ADDITY_CTL_OP_MIN = 0x41646421, /* A + d + d + ! */
	DCMD_FSYS_ADDITY_CTL_OP_ADD,              /* 0x41646422 : add partition/disk to addity */
	DCMD_FSYS_ADDITY_CTL_OP_LIST,             /* 0x41646423 : list addity members */
	DCMD_FSYS_ADDITY_CTL_OP_MAX               /* 0x41646424 */
};
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/blk/io-blk/public/sys/dcmd_blk.h $ $Rev: 911261 $")
#endif
