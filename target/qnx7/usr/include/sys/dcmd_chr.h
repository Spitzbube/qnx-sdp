...
/*
 * Desc:   This call is made to get the discard output state.
 * Args:   A pointer to an unsigned integer that is filled in by the terminal device
 */
#define DCMD_CHR_GET_DISCARD_OUTPUT	__DIOF(_DCMD_CHR, 40, unsigned int)

/*
 * Desc:   This call is made to cause all data written to the device to be discarded until this condition
 *         is cleared.
 * Notes:  This command is restricted using euid/egid permission checking.
 * Args:   A pointer to an unsigned integer containing 1 to enable discard and 0 to clear discard.
 */
#define DCMD_CHR_SET_DISCARD_OUTPUT	__DIOT(_DCMD_CHR, 41, unsigned int)
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/io-char/public/sys/dcmd_chr.h $ $Rev: 837977 $")
#endif
