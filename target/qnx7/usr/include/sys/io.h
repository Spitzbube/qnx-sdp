...
#define IO_FLAG_VU_DATA		0x2000	/* IOReq has vendor data in "user" */
...
#define IO_FLUSH_NOSYNC     0x0010  /* don't send SYNC cmd to disk driver */
...
typedef struct _cam_smmu_map CAM_SMMU_MAP;
...
struct _io_entry {
	...
	int			(*io_smmu)(int (*map)(CAM_SMMU_MAP *maplist));
};

#define _IO_ENTRY_NFUNCS	((sizeof(io_entry_t) - offsetof(io_entry_t, io_init)) / sizeof(void *))
#define _IO_ENTRY_FUNC_VALID( _entry_struct, _fcn )	(((_entry_struct->nfuncs) >= (((offsetof(io_entry_t, _fcn) - offsetof(io_entry_t, io_init)) / sizeof(void *)) + 1)) &&  (_entry_struct->_fcn != NULL))
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/blk/io-blk/public/sys/io.h $ $Rev: 915926 $")
#endif
