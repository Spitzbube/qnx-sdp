...
typedef enum
{
	 ...
	 * functionality
	 *
	 * 1. there are other API's besides pci_device_attach() that trigger a connection
...
typedef enum
{
	...
	/* non-specification defined resets */
	...
	/* HW specific resets, the behaviour of which are defined by the
	 * implementing HW module */
 ...
 * otherwise */
pci_device_type_e pci_device_get_type(const pci_bdf_t bdf, uint_t *sub_type);
...
*/
pci_bdf_t pci_chassis_slot_device(pci_cs_t cs, pci_err_t *err);
...
/*
 ...
 1. Client starts after pci-server (normal scenario)
	- if client calls to API's which trigger a connection to the PCI server
 ...
 2. Client starts before pci-server
	- client calls to API's which trigger a connection to the PCI server fail
 ...
 2. Asynchronous disconnect
	...
	the API's defined in pci/pci_event.h
	   ...
	   a designated client not attached to devices
		...
		- On reception of EXITING event
		  ...
		  - attached clients inform the designated client that they have completed
			detachment and cease to use any other libpci API's
		- On reception of TERMINATING event
		  ...
		  - It is user defined when clients begin to start using libpci API's
			again
		- Subsequent connection initiation attempts operate as described under
		  Connection Initiation
	...
	situation until attempts to communicate with the server (via libpci API's)
	...
	a. pci-server terminates (normally or abnormally)
		...
		- all attached clients that use API's which require sending a message
		...
		- clients should cease to use any libpci API's. It is user defined when
		  clients begin to start using libpci API's again
		- Subsequent connection initiation attempts operate as described under
		  Connection Initiation
	...
	- should cease calling libpci API's (except for pci_device_detach() for
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/pci/mainline/lib/public/pci/pci.h $ $Rev: 893964 $")
#endif
