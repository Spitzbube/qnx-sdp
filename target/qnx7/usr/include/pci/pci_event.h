...
/*
 ...
 For each event, the following event specific information is provided
	 ...
	 sigev_code value: an event specific value for the 'sigev_code' field of the
					   ...
					   pci_event_attr_get_sigev_code() API's respectively.
...
pci_event_type_e_SERVER_first = 1,
	 ...
	 * Registration ID: pci_event_regid_t_INTERNAL
	 * Handler Data Field: server_internal.state
...
pci_event_type_e_NTB_first_event = pci_event_type_e_NTB_first,
	 ...
	 * Registration ID: an event source identifier such as that returned from
	 *                  a successful call to pci_ntb_evt_srcid_alloc()
...
typedef union __attribute__((aligned((8))))
{
	...
	union
	{
		/* for server state change events, the current and next state will be
		 * reported */
		struct pci_server_state_s
		{
			uint8_t current;
			uint8_t next;
		} state;	/* pci_server_state_e */
	} server_internal;
	struct
	{
		/*
		 ...
		 *
		 * 'id' is used to uniquely identify a 'pci_err_record_t' and valid
		 ...
		 * different clients should that be desired
		 */
...
typedef uint64_t pci_ntb_evt_srcid_t;
#define pci_event_regid_t_INTERNAL	(UINT64_MAX - 1)
typedef union
{
	pci_bdf_t bdf;
	pci_cs_t cs;
	pci_ntb_evt_srcid_t evt_srcid;
	uint64_t internal;
...
/*
 ...
 handler specific meaning to different values and use the API's to set and get
 the value.
...
*/
pci_err_t pci_event_attr_set_sigev_code(pci_event_attr_t *event_attr, const int_t code);
pci_err_t pci_event_attr_get_sigev_code(const pci_event_attr_t event_attr, int_t *code);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/pci/mainline/lib/public/pci/pci_event.h $ $Rev: 914339 $")
#endif
