...
/***** defines **************/
#define ADO_MAJOR_VERSION          6
#define AFM_MAJOR_VERSION          5
#define APX_MAJOR_VERSION          2

/**** Convenience Macro's ****/
#define ADO_ROUND_UP(n, sz) (((n) + ((sz) - 1)) & ~((sz) - 1)) /* sz must be a power of 2 */
#define ADO_ROUND_DOWN(n, sz) (n & ~((sz) - 1)) /* sz must be a power of 2 */
#define ADO_CONTIG_VOICE_MASK(voices) ((uint32_t)((1ULL << (voices)) - 1ULL)) /*use int64_t so the 32 voice case doesn't overflow */
#define ADO_MSEC_TO_BYTES(ms, rate, byte_width, voices) (ms * (rate/1000) * byte_width * voices) /* PCM data size in bytes based on the duration and audio format */
...
typedef void    (ado_dll_version_t) (int *major, int *minor, const char **date);
...
typedef int     (ado_ctrl_dll_devctl_t) (uint32_t cmd, uint8_t *msg, uint16_t *msg_size, HW_CONTEXT_T *context);

typedef int32_t (ado_restore_dll_init_t) (ado_card_t * card);
...
uint32_t        sw_mixer_samples;           /* sw mixer fragment has sw_mixer_ms samples per voice */
rate_method_t   sw_mixer_rate_method;       /* method used to determine the sw mixer sample rate */
uint32_t        sw_mixer_rate_rate;         /* sample rate used by sw mixer in the case sw_mixer_rate_method is set to RM_LOCKED */
uint16_t        sw_mixer_ms;                /* alternative to sw_mixer_samples, duration of one sw mixer fragment in ms, has precedence over sw_mixer_samples */
uint16_t        line_perm_override;         /* override phoneline permission check */
uint16_t        sw_mixer_sample_size;       /* sample size the sw mixer operates at (16 or 32) */
uint16_t        smmu:1;                     /* Use the SMMU (System Memory Management Unit)    */
};
...
const ado_dict_t *ado_get_card_dict( ado_card_t *card );
void ado_card_lock( ado_card_t *card );
void ado_card_unlock( ado_card_t *card );
#ifdef HAVE_SMMU_H
struct smmu_object *ado_card_smmu( ado_card_t *card);
#endif
...
/* Untracked interrupt handlers can avoid thread contention and possibly
 * avoid deadlocks due to interdependencies. Thus it is better to use
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/services/audio/public/include/ado_core.h $ $Rev: 913635 $")
#endif
