...
enum virtio_device_type {
	...
	VIRTIO_DT_TIMER,
	VIRTIO_DT_INPUT,
	VIRTIO_DT_UNUSED_ID19,
	VIRTIO_DT_CRYPTO,
	VIRTIO_DT_NUM,

	VIRTIO_DT_QNX_PRIVATE=0xf000,
	VIRTIO_DT_QNX_AUDIO=VIRTIO_DT_QNX_PRIVATE, /* obsoleted */
...
#define VIRTIO_ANY_VENDOR       ((uint16_t)0xffffu)

/**
 * Combine an experimental device type with a sub-system vendor id.
 * The result can then be passed to viod_find_pci() and viod_find_mmio().
 * @param   did     VIRTIO_DT_* value
 * @param   vid     sub-system vendor id to match, possibly VIRTIO_ANY_VENDOR
 * @return  a value suitable for viod_find_pci() and viod_find_mmio().
 */
static inline enum virtio_device_type
virtio_dt_combined(enum virtio_device_type const did, uint16_t const vid)
{
    return (enum virtio_device_type)((uint32_t)did | ((uint32_t)vid << 16));
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/hyp/public/virtio.h $ $Rev: 876894 $")
#endif
