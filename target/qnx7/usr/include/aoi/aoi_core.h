...
#define AOI_MAJOR_VERSION 1
#define AOI_MINOR_VERSION 2
#define AOI_PATCH_VERSION 0

void AoLibraryVersion(int32_t *major, int32_t *minor, int32_t *patch);
...
enum AOStringSearchFlags { // flags for AoSearchString()
	AO_STRING_IGNORE_CASE = 1,
};
...
const AOICtrl_t *AoIterateName(const char *name, const char *iname, int32_t version, int32_t *cookie);
int32_t AoAddUnloadSignal(int sig);
int32_t AoSaveConfig(const char *config_file, uint32_t options);
int32_t AoLoadConfig(const char *config_name, AOLoadedConfig_t **cfg);
...
int AoSearchString(const char *list, const char *val, size_t val_len, unsigned flags);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/aoi/public/aoi/aoi_core.h $ $Rev: 898566 $")
#endif
