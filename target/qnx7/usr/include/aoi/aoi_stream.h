...
#define AOSTREAMER_HTTPINFO_VERSION 100
/*
 * The AOStreamerHTTPInfo interface provides HTTP headers.
 */
typedef struct {
  const char *(*GetHeaders)(void *ctx);
} AOStreamerHTTPInfo;
...
#define AOSTREAMER_MIMEINFO_VERSION 100

typedef struct
{
  const char *mimetype;
  const char *extension;
} AOStreamerMimeInfo_t;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/lib/aoi/public/aoi/aoi_stream.h $ $Rev: 886192 $")
#endif
