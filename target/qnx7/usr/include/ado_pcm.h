...
struct ado_pcm_subchn_mixer_config
{
	...
	uint32_t							volume_dB_scale_factor;
	...
	void		(*balance_set) (HW_CONTEXT_T *hw_context, PCM_SUBCHN_CONTEXT_T *pcm_sc_context,  uint32_t * volumes, int32_t mute, uint32_t *balance_levels, uint32_t *fade_levels, ado_pcm_subchn_mixer_config_t *config);
#if (__PTR_BITS__ == 32)
	uint32_t							spare_2[14];
#else
	uint32_t							spare_2[13];
...
uint32_t ado_chmap_to_mixer_chn_mask(const snd_pcm_chmap_t *chmap, const uint32_t voices);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/BC700_6762_sdp704/services/audio/public/include/ado_pcm.h $ $Rev: 885943 $")
#endif
