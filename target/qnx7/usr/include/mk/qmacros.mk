...
cpus=x86_64 arm aarch64 aarch64v8_2 $(EXTRA_CPU_NAMES)
oses=nto win64 linux unix darwin $(EXTRA_OS_NAMES)
...
CXXVFLAGS=$(VFLAGS) $(foreach var,$(VARIANT_LIST), $(CXXVFLAG_$(var))) $(addprefix -D,$(VARIANT_NAMES))
...
# Used to know which compiler version we're using because
# sometimes you need to enable options on one version which
# aren't supported on others
ifndef COMPILER_VERSION
ifdef COMPILER_VERSION_$(OS)_$(CPU)
COMPILER_VERSION=$(COMPILER_VERSION_$(OS)_$(CPU))
else
ifdef COMPILER_VERSION_$(OS)
COMPILER_VERSION=$(COMPILER_VERSION_$(OS))
else
ifdef GCC_VERSION
COMPILER_VERSION=$(shell echo $(GCC_VERSION) | sed -E 's/[[:blank:]\.]+//g')
else # GCC_VERSION
ifeq ($(COMPILER_DRIVER),qcc)
COMPILER_VERSION=$(shell qcc -V 2>&1 | grep default | sed -e 's/,.*//' | sed -E 's/[[:blank:]\.]+//g')
else # qcc
ifeq ($(COMPILER_TYPE),gcc)
COMPILER_VERSION=$(shell $(firstword $(CCPREF)) --version | sed 1q | sed 's/^.* \(.*\)/\1/' | sed -E 's/[[:blank:]\.]+//g')
else
COMPILER_VERSION=0
endif # gcc
endif # qcc
endif # GCC_VERSION
endif # COMPILER_VERSION_$(OS)
endif # COMPILER_VERSION_$(OS)_$(CPU)
endif # COMPILER_VERSION

# Macros to test the compiler version
# These functions return 1 if the test is true
# ifeq ($(call COMPILER_VERSION_[GE|LT], 540),1)
COMPILER_VERSION_GE=$(if $(filter 0, $(shell test $(COMPILER_VERSION) -ge $(1); echo $$?)),1,0)
COMPILER_VERSION_LT=$(if $(filter 0, $(shell test $(COMPILER_VERSION) -lt $(1); echo $$?)),1,0)
...
QCC_CONF_VARIANT=$(if $(filter arm,$(CPU)),$(CPU)v7)
endif
...
# SRCVERSION Tracking using GCC plugin srcversion.so
# Only enabled on NTO builds
ifeq ($(OS),nto)
ifdef SRCVERSION_TRACK
# Path to SRCVERSION plugin
ifeq ($(SRCVERSION_PLUGIN),)
$(error SRCVERSION_PLUGIN is not defined: plugin full path)
endif
# SRCVERSION Build ID tracking value
ifeq ($(SRCVERSION_BUILDID),)
$(error SRCVERSION_BUILDID is not defined: build ID used for tracking this component)
endif
# Source tree base folder path
ifeq ($(SRCVERSION_BASEPATH),)
$(error SRCVERSION_BASEPATH is not defined: path to base of source tree)
endif
SRCV_FLAGS+=-fplugin=$(SRCVERSION_PLUGIN)
SRCV_FLAGS+=-fplugin-arg-srcversion-path=$(QNX_TARGET)
SRCV_FLAGS+=-fplugin-arg-srcversion-path=$(SRCVERSION_BASEPATH)
SRCV_FLAGS+=-fplugin-arg-srcversion-path=$(INSTALL_ROOT_$(OS))
SRCV_FLAGS+=-fplugin-arg-srcversion-path=$(QNX_HOST)
SRCV_FLAGS+=-fplugin-arg-srcversion-buildid=$(SRCVERSION_BUILDID)
CCFLAGS+=$(SRCV_FLAGS)
CXXFLAGS+=$(SRCV_FLAGS)
endif
endif
...
COMPILE_c_o=$(CCWRAP) $(CCPREF) $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $< $(CCPOST)
COMPILE_cc_o=$(CCWRAP) $(CCPREF) $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $< $(CCPOST)
COMPILE_cxx_o=$(CCWRAP) $(CCPREF) $(CXXFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CXXOPTS) $< $(CCPOST)
COMPILE_nda=$(CP_HOST) $< $@

COMPILE_S_i=$(ASWRAP) $(ASPREF) -E $(ASFLAGS) $(FLAGS) $(ASVFLAGS) $(ASOPTS) $< >$@ $(ASPOST)
COMPILE_c_i=$(CCWRAP) $(CCPREF) -E $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $< >$@ $(CCPOST)
COMPILE_cc_i=$(CCWRAP) $(CCPREF) -E $(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $< >$@ $(CCPOST)
COMPILE_cxx_i=$(CCWRAP) $(CCPREF) -E $(CXXFLAGS) $(FLAGS) $(CXXVFLAGS) $(CPPFLAGS) $(CXXOPTS) $< >$@ $(CCPOST)
...
isr_infix=$(filter-out $(CCFLAGS_ISR_EXCLUDE) $(CCVFLAGS_ISR_EXCLUDE),$(CCFLAGS) $(FLAGS) $(CCVFLAGS) $(CPPFLAGS) $(CCOPTS) $(CCFLAGS_ISR) $(CCVFLAGS_ISR))
